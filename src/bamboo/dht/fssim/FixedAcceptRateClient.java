...
public class FixedAcceptRateClient extends RandomUtil {
    protected long next_new_put;
    public FixedAcceptRateClient (final int client, final long period, 
                                  final int size, final int ttl_sec, 
                                  long start, final long stop_time, long seed,
                                  final long retry_period) {
                    ...
                    alg.enqueue_put (client, size, ttl_sec, 
                        new Algorithm.PutResultCb () {
                            public void cb (boolean success, boolean again) {
                                if (! success) {
                                    long retry_wait = Math.round (Math.ceil (
                                            random_gaussian (retry_period, 
                                                retry_period * 0.1, rand)));
                                    if (sim.now_ms () + retry_wait <
                                        next_new_put) {
                                        alg.enqueue_put (client, size, ttl_sec, 
                                            this, null);
                                    }
