 ...
 *
 * @author Sean C. Rhea
 * @version $Id: OldStorageManager.java,v 1.1 2005/06/06 23:42:33 srhea Exp $
 */
public class OldStorageManager extends StandardStage
implements SingleThreadedEventHandlerIF {
    ...
    public static final Key ZERO_KEY =
        new Key (0, 0, ZERO_GUID, ZERO_HASH, false, ZERO_CLIENT);
    ...
    protected void storage_changed (final boolean added, final Key fk, 
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("storage changed: " + (added ? "added" : "removed") +
                         " key " + fk + " of " + size + " bytes");
        }
        ...
        if (added) 
            bb.putLong (old_usage + size + Key.SIZE);
        else
            bb.putLong (old_usage - size - Key.SIZE);
    ...
    public void notify_storage_changed (final StorageMonitor sm, 
        ...
        acore.register_timer(0, new ASyncCore.TimerCB() {
            ...
            long sz = size;
            public void timer_cb(Object ud) { mon.storage_changed(a, c, sz); }
        }, null);
    }
	...
	public Key (long t, int tt, BigInteger g, byte [] dh,
	...
	public Key (ByteBuffer buf) {
        ...
        public Key (InputBuffer buf) {
            if (buf instanceof NioInputBuffer)
                init (((NioInputBuffer) buf).bb);
            else {
        ...
        public void serialize (OutputBuffer buf) {
            if (buf instanceof NioOutputBuffer)
                to_byte_buffer (((NioOutputBuffer) buf).bb);
            else {
                buf.add (time_usec);
		buf.add (ttl_sec);
	...
	public int compareTo (Object rhs) {
        ...
        public boolean equals (Object rhs) {
            OldStorageManager.Key other = (OldStorageManager.Key) rhs;
            return compareTo (rhs) == 0;
        }

	public String toString () {
	    return "(Key time_usec=" + Long.toHexString (time_usec) +
		" ttl_sec=" + ttl_sec + " guid=" +
		GuidTools.guid_to_string (guid) + " data_hash=" +
		ByteUtils.print_bytes (data_hash, 0, 3) + " put=" + put +
                " client_id=" + client_id.getHostAddress() + ")";
	}
    }
    ...
    public static class IterateByGuidResp implements QueueElementIF {
        public OldStorageManager.Key key;
        public ByteBuffer data;
        public Object continuation;
        public Object user_data;
        public IterateByGuidResp (OldStorageManager.Key k, ByteBuffer d, Object c, Object ud) {
            key = k; data = d; continuation = c; user_data = ud;
        }
        public String toString () {
	...
	public Dbc cursor;
	public IBGCont (Dbt k, BigInteger h, Dbc c) {
	    key = k; high = h; cursor = c;
	}
    }
	...
	// Initialize tables
        ...
        */
        ...
        Key k = new Key (0, 0, ZERO_GUID, ZERO_HASH, false, ZERO_CLIENT);
        ...
        if (! sim_running) {
            ...
            t.setName ("OldStorageManager.BerkeleyDbThread");
            t.start ();
        }
            ...
            if (sim_running) {
                ...
                if (count > 0) { 
                    if (now_ms - last_ms > 180*1000) {
                        logger.fatal ("BerkeleyDB thread blocked. "
                                + " now_ms=" + now_ms
                    ...
                    if (now_ms - last_ms > 60*1000) {
                        logger.warn ("BerkeleyDB thread appears blocked. "
                                + " now_ms=" + now_ms
                                + " last_dequeue_ms=" + last_ms
                                + ".  There are at least " + count
                                + " outstanding requests.");
	...
	// whichever existing datum it overrides, if any.  Also, we have to
	// transaction-protect the whole sequence in order to maintain the
	// property that there is at most one put and remove each in the
	// database.  We do specify DB_TXN_NOSYNC, however, so that we get
	// atomicity, consistency, and isolation, but not necessarily
	// durability (in return for increased performance).  This tradeoff
	// is safe since if the tuple is lost, it will be recovered by the
	// epidemic algorithms in Bamboo.
        ...
        Dbt skey = null;
        {
            ...
            System.arraycopy(req.key.data_hash, 0, buf, 20, 20);
            skey = new Dbt(buf);
        ...
        while (retval != Db.DB_NOTFOUND) {
            ...
            Key k = new Key (ByteBuffer.wrap (pkey.getData (),
                                              pkey.getOffset(),
                                              pkey.getSize ()));
            if (logger.isDebugEnabled ())
            ...
            if (req.key.compareTo (k) == 0) {
                ...
                if (req.key.put && k.put) {
                    if (req.key.compareTo (k) < 0) {
                        // This put is older than the existing put.
                        do_put = false;
                        inval_put_key = req.key;
                        inval_put_data = req.data;
                    }
                    else {
                        // Replace the existing put.
                        inval_put_key = k;
                ...
                if (req.key.put && (! k.put)) {
                    if (req.key.compareTo (k) < 0) {
                        // This put is older than the existing remove.
                        do_put = false;
                        inval_put_key = req.key;
                        inval_put_data = req.data;
                    }
                    ...
                    if (req.key.compareTo (k) < 0) {
                        // This remove is older than the existing remove.
                        do_put = false;
                    ...
                    if (req.key.compareTo (k) < 0) {
                        // This remove is older than the existing put.
                        // Do nothing.
                    }
                    else {
                        // Remove the existing put.
                        inval_put_key = k;
                        inval_put_data = ByteBuffer.wrap (data.getData (),
                                                          data.getOffset (),
                                                          data.getSize ());
	...
	// invalidate.  Now we do the put if it's still necessary.
        ...
        if (do_put) {
            ...
            if (logger.isInfoEnabled ()) {
                ...
                buf.append ("s client_id=");
                buf.append (req.key.client_id.getHostAddress ());
                buf.append (" data_hash=0x");
                buf.append (ByteUtils.print_bytes (
                            req.key.data_hash, 0, 4));
                buf.append ("size=");
                buf.append (data.getSize ());
                if (inval_put_key != null) {
                    buf.append (" overwrites old put data_hash=0x");
                    buf.append (ByteUtils.print_bytes (
                                inval_put_key.data_hash, 0, 4));
                }
                if (inval_rm_key != null) {
                    if (inval_put_key != null)
                        buf.append (" and rem data_hash=0x");
                    else
                        buf.append (" overwrites old rem data_hash=0x");
                    buf.append (ByteUtils.print_bytes (
                                inval_rm_key.data_hash, 0, 4));
                }
                logger.info (buf);
            }
        }
	...
	return ((k.time_usec + ((long) k.ttl_sec) * 1000000) <
                ((long) now_ms() * 1000));
    }
    ...
    protected void handle_get_by_guid_req (GetByGuidReq req) {
        ...
        if ((k == null) || (k.equals (ZERO_KEY))) {
            ...
            k = new Key (0L, 0, req.guid, ZERO_HASH, false, ZERO_CLIENT);
        }
    ...
    protected boolean finish_get_by_guid(GBGCont cont, DbTxn xact, Dbc cursor,
        ...
        Key k = new Key (ByteBuffer.wrap (pkey.getData(), pkey.getOffset(),
                                          pkey.getSize()));
        if (key_expired(k)) {
            // drop it, grab next
            if (logger.isDebugEnabled ())
    ...
    protected void handle_get_by_time_req (GetByTimeReq req) {
        Key k = new Key (req.low, 0, ZERO_GUID, ZERO_HASH, false, ZERO_CLIENT);
        byte [] kbuf = new byte [Key.SIZE];
        ByteBuffer bb = ByteBuffer.wrap (kbuf);
        k.to_byte_buffer (bb);
        Dbt key = new Dbt (kbuf);
    ...
    protected void finish_get_by_time (GBTCont cont, DbTxn xact, Dbc cursor,
        ...
        while (true) {
            ...
            Key k = new Key (ByteBuffer.wrap (cont.key.getData (),
                        cont.key.getOffset (),
                        cont.key.getSize ()));
    ...
    protected void handle_iterate_by_guid_req (IterateByGuidReq req) {
        Key k = new Key (0L, 0, req.low, ZERO_HASH, false, ZERO_CLIENT);
        byte [] kbuf = new byte [Key.SIZE];
        ByteBuffer bb = ByteBuffer.wrap (kbuf);
        k.to_byte_buffer (bb);
        ...
        IBGCont cont = new IBGCont (key, req.high, cursor);
        int retval = 0;
        Dbt pkey = new Dbt ();
        Dbt data = new Dbt ();
        ...
        while (! finish_iterate_by_guid (cont, pkey, data, retval, req.comp_q,
                                         req.user_data)) {
           try { retval = cursor.get(key, data, Db.DB_NEXT); }
           catch (DbException e) { BUG(e); }
        }
    }
    ...
    protected void handle_iterate_by_guid_cont (IterateByGuidCont req) {
        ...
        if (req.comp_q == null) {
            ...
            do {
                try {
                    retval = cont.cursor.get(cont.key, pkey, data,
                                                 Db.DB_NEXT);
                }
                catch (DbException e) { BUG(e); }
            }
            while (! finish_iterate_by_guid (cont, pkey, data, retval,
                                             req.comp_q, req.user_data));
        }
    }

    protected boolean finish_iterate_by_guid (IBGCont cont, Dbt pkey,
                                              Dbt data,
                                              int retval,
                                              SinkIF comp_q, Object user_data) {
    ...
    protected void handle_discard_req (DiscardReq req) {
        ...
        if (retval == 0) {
            ...
            if (logger.isInfoEnabled ()) {
                ...
                sbuf.append (" data_hash=0x");
                sbuf.append (ByteUtils.print_bytes (req.key.data_hash, 0, 4));
                sbuf.append ("size=");
